...
class SpreadButton: UIView, CAAnimationDelegate {
    ...
    fileprivate static let flowerSpreadAngleDefault: CGFloat = 90.0
    ...
    func configureMainButton(_ image: UIImage, highlightImage: UIImage?) {
        ...
        powerButton.addTarget(self, action: #selector(tapPowerButton(_:)), for: .touchUpInside)
    ...
    func spreadSubButton() {
        ...
        for btn in self.subButtons! {
            ...
            if btnIndex == 0 {
                startOutSidePoint = calculatePoint(angle, radius: radius, btnIndex: btnIndex)
                startAngle = angle
            }
            
            let outsidePoint = calculatePoint(angle, radius: radius, btnIndex: btnIndex)
            let shockOutsidePoint = calculatePoint(angle, radius: radius + 10, btnIndex: btnIndex)
            let shockInsidePoint = calculatePoint(angle, radius: radius - 3, btnIndex: btnIndex)
            ...
            case .spreadModeLineSpread:
                ...
                case .spreadDirectionLeft, .spreadDirectionLeftDown, .spreadDirectionLeftUp:
                    animationPath = movingPath(btn.layer.position, keyPoints: outsidePoint)
                case .spreadDirectionTop:
                    animationPath = movingPath(btn.layer.position, keyPoints: outsidePoint)
                case .spreadDirectionRight, .spreadDirectionRightUp, .spreadDirectionRightDown:
                    animationPath = movingPath(btn.layer.position, keyPoints: outsidePoint)
                }
...
//                positionAnimation.duration = animationDuringSpread
    ...
    func calculatePoint(_ angle: CGFloat, radius: CGFloat, btnIndex: Int?) -> CGPoint {
        //根据弧度和半径计算点的位置
        //center => powerButton
        switch mode {
        case .spreadModeFlowerSpread, .spreadModeSickleSpread:
            return CGPoint(x: powerButton.center.x + cos(angle/180.0 * π) * radius, y: powerButton.center.y - sin(angle/180.0 * π) * radius)
        case .spreadModeLineSpread:
            switch direction {
            case .spreadDirectionTop:
                return CGPoint(x: powerButton.center.x, y: powerButton.center.y - CGFloat((btnIndex! + 1)) * radius)
            case .spreadDirectionLeft, .spreadDirectionLeftDown, .spreadDirectionLeftUp:
                return CGPoint(x: powerButton.center.x  - CGFloat(btnIndex! + 1) * radius, y: powerButton.center.y)
            case .spreadDirectionBottom:
                return CGPoint(x: powerButton.center.x, y: powerButton.center.y + CGFloat((btnIndex! + 1)) * radius)
            case .spreadDirectionRight, .spreadDirectionRightUp, .spreadDirectionRightDown:
                return CGPoint(x: powerButton.center.x  + CGFloat(btnIndex! + 1) * radius, y: powerButton.center.y)
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        cover.frame = newSuperview?.bounds ?? CGRect.zero
    }
    
    override func didMoveToSuperview() {
        animator = UIDynamicAnimator(referenceView: self.superview ?? self)
    }
    ...
    func setSubButtons(_ buttons: [SpreadSubButton?]) {
        _ = buttons.compactMap { $0?.addTarget(self, action: #selector(clickedSubButton(_:)), for: .touchUpInside) }
